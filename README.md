...
| 🖼️ **A photo for every AI recipe** | After the recipe is written, an image model photographs the dish and a vision model checks it actually shows that dish with nothing odd in it; only then does it become the cover (captioned "AI photo"). One stricter retry, then the emoji stays. Off by a toggle; needs a vendor that can make images (OpenAI). |
