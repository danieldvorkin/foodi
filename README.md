...
| 🖼️ **A photo for every recipe** | After the AI writes a recipe it gets a cover: OpenAI accounts generate one (checked by a vision model before it's shown, captioned "AI photo"); everyone else gets a photo of the dish from a photo library — Pexels or Google Images when a key is set, Wikimedia Commons always — credited under the photo and vetted by the person's own vision model when they have one. "Try another photo" keeps searching until something passes (the dish first, plain food as the fallback, rejects remembered), with a strip of the ones tried to jump back, × to delete any, and "Generate with AI" when the vendor can. Admins can backfill the house kitchen in one click. Off by a toggle. |
...
| 🛍️ **Shop** | Sell anything cooking-adjacent — gear, jars, books, classes, services — with photos, stock and a delivery note from the buyer. Every listing (and every edit) is reviewed by a person before it goes live; sellers mark orders as sent. |
| 🔐 **Admin permissions** | Granular rights on top of the admin role: today `posting-approvals` (approve/reject Shop listings). Admins without it can see the queue and take listings down, but not decide. Granted per person under Admin → People. |
...
| 🔑 **Connect your AI, guided** | A four-step illustrated walkthrough for each vendor — create the developer account (not the same as ChatGPT/Claude), add $5 of credit (~2¢ a recipe), create a key, paste — with a button to the exact page at each step and ticks that remember your place. The key is checked with one tiny real request, so an account with no credit is told so before it ever fails on a recipe. Optional, off by default: with an OpenAI org admin key the operator can have foodi issue keys automatically instead (Admin → Settings). |
| 🛒 **Shopping list** | One list that fills itself: "Add to shopping list" on any recipe picks the servings and the lines you need (garnishes off by default), scales the numbers and merges into lines you already have (2 lemons + 3 lemons = 5). Quick-add by hand ("500 g flour"). Grouped by aisle from the ingredient library, big one-tap rows for the shop, ticked lines sink, clear ticked or all. |
| 📅 **Meal plan** | A Monday-first week, meal by meal (breakfast, lunch, dinner, snack) with a Prep lane for batch-cooking days. Fill a slot from your recipes or the house kitchen, or just type "leftovers"; open an entry to mark it cooked, change servings, move it, add a note, or start cooking. "Add this week to the list" puts every planned recipe's ingredients on the shopping list at the planned servings, merged. Deleted recipes keep their name on the plan. The feed shows what's coming up. |
| 🪟 **Browse without an account** | `/browse` is the shop window: every public recipe, searchable and filterable by meal, diet, time and effort, no session needed. A recipe page shows the photo, summary, timings, ingredients and step *titles* — the method itself, saving, cooking and the shopping list are the moment it asks you to join, and you land back on that recipe after signing up. Link previews and the sitemap point here so crawlers and friends see a real page. |
| 🔗 **Links that unfurl** | Paste a foodi link anywhere and it shows a card: the site card for the front page, and for a public recipe, person, book, blog post or shop listing its own title, blurb and photo. Installable on a phone (manifest + icons), `robots.txt` and a sitemap of public pages. Assets to share by hand live in `docs/marketing/`. |
