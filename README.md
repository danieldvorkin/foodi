...
| ✨ **Zero-setup AI** | With an OpenAI organisation admin key, foodi gives every new person their own API key the moment they finish onboarding (a service account in your org, billed to you, capped per day in Admin). One tap on the connect page does the same on demand. Bringing your own key retires foodi's. Without the admin key, people connect their own OpenAI or Claude key — a two-step flow with an "open the key page" button and paste. |
